...
public class sales_report extends javax.swing.JInternalFrame {
    ...
    void load_report(String dfrom, String dto) {
        try {
            ...
            String query;
            if (all.isSelected()) {
                query = "select billno,date_format(dat,'%d/%m/%Y'),tim,location,terminal,cashier,items,quans,sub,disp,disamt,gross,taxamt,addamt,net,pby,paid,bal,price_type,cid,cname,mobile,last from sales where dat between '" + lk + "' and '" + lk1 + "'" + companyFilter + " order by dat,billno";
            } else {
                query = "select billno,date_format(dat,'%d/%m/%Y'),tim,location,terminal,cashier,items,quans,sub,disp,disamt,gross,taxamt,addamt,net,pby,paid,bal,price_type,cid,cname,mobile,last from sales where dat between '" + lk + "' and '" + lk1 + "' and pby='" + h3.getSelectedItem().toString() + "'" + companyFilter + "  order by dat,billno";
            }
