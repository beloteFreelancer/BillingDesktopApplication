...
public class estimate_report extends javax.swing.JInternalFrame {
    ...
    void load_report(String dfrom, String dto) {
        try {
            ...
            if (all.isSelected()) {
                query = "select billno,date_format(dat,'%d/%m/%Y'),tim,location,terminal,cashier,items,quans,sub,disp,disamt,gross,taxamt,addamt,net,pby,paid,bal,price_type,cid,cname,mobile,last from estimate where dat between ? and ?"
                        + companyFilter + " order by dat,billno";
                ps = conn.prepareStatement(query);
                ps.setString(1, lk);
                ps.setString(2, lk1);
            } else {
                query = "select billno,date_format(dat,'%d/%m/%Y'),tim,location,terminal,cashier,items,quans,sub,disp,disamt,gross,taxamt,addamt,net,pby,paid,bal,price_type,cid,cname,mobile,last from estimate where dat between ? and ? and pby=?"
                        + companyFilter + " order by dat,billno";
                ps = conn.prepareStatement(query);
                ps.setString(1, lk);
                ps.setString(2, lk1);
            ...
            while (r.next()) {
                ...
                s2.addRow(new Object[] { r.getString(1), r.getString(2), r.getString(3), r.getString(4), r.getString(5),
                        r.getString(6), r.getString(7), r.getString(8), sub, r.getString(10), disamt, gross, taxamt,
                        other, net, r.getString(16), paid, bal, r.getString(19), "  " + r.getString(20),
