...
public class stock_report extends javax.swing.JInternalFrame {
    ...
    void load_report() {
        try {
            ...
            if (all.isSelected()) {
                query = "select ino,iname,sum(quan),sum(quan*prate),sum(quan*mrp),sum(quan*rprice),sum(quan*wprice) from stock where quan>0"
                        + companyFilter + " group by ino order by ino";
            } else {
                query = "select ino,iname,sum(quan),sum(quan*prate),sum(quan*mrp),sum(quan*rprice),sum(quan*wprice) from stock where cat='"
                        + h3.getSelectedItem() + "' and quan>0" + companyFilter + " group by ino order by ino";
            }
