...
public class purchase_summary_iname extends javax.swing.JInternalFrame {
    ...
    void load_report(String dfrom, String dto) {
        try {
            ...
            if (all.isSelected()) {
                query = "select ino,iname,sum(quan),sum(amount) from purchase_items where bdate between ? and ?" + companyFilter + " group by ino,iname order by iname";
                ps = conn.prepareStatement(query);
                ps.setString(1, lk);
                ps.setString(2, lk1);
            } else {
                query = "select ino,iname,sum(quan),sum(amount) from purchase_items where bdate between ? and ? and iname=?" + companyFilter + " group by ino,iname order by iname";
                ps = conn.prepareStatement(query);
                ps.setString(1, lk);
                ps.setString(2, lk1);
    ...
    final void get_cid() {
        try {
            ...
            PreparedStatement ps = conn.prepareStatement("select count(distinct iname) from purchase_items" + companyFilter);
            r = ps.executeQuery();
            while (r.next()) {
                count = r.getInt(1);
            }
            ps = conn.prepareStatement("select distinct iname from purchase_items" + companyFilter);
            r = ps.executeQuery();
    ...
    void get_cname() {
        try {
            ...
            Connection conn = util.getConnection();
            String companyFilter = UserSession.hasSelectedCompany() ? " and grn in (select grn from purchase where company_id='" + UserSession.getSelectedCompanyID() + "')" : "";
            PreparedStatement ps = conn.prepareStatement("select distinct ino from purchase_items where iname=?" + companyFilter);
