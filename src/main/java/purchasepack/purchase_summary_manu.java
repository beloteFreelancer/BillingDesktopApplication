...
public class purchase_summary_manu extends javax.swing.JInternalFrame {
    ...
    void load_report(String dfrom, String dto) {
        try {
            ...
            if (all.isSelected()) {
                query = "select manu,sum(quan),sum(amount) from purchase_items where bdate between ? and ?" + companyFilter + " group by manu order by manu";
                ps = conn.prepareStatement(query);
                ps.setString(1, lk);
                ps.setString(2, lk1);
            } else {
                query = "select manu,sum(quan),sum(amount) from purchase_items where bdate between ? and ? and manu=?" + companyFilter + " group by manu order by manu";
                ps = conn.prepareStatement(query);
                ps.setString(1, lk);
                ps.setString(2, lk1);
    ...
    final void get_cid() {
        try {
            ...
            PreparedStatement ps = conn.prepareStatement("select count(distinct manu) from purchase_items" + companyFilter);
            r = ps.executeQuery();
            while (r.next()) {
                count = r.getInt(1);
            }
            ps = conn.prepareStatement("select distinct manu from purchase_items" + companyFilter);
            r = ps.executeQuery();
