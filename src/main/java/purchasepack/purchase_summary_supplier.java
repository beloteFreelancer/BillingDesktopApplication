...
public class purchase_summary_supplier extends javax.swing.JInternalFrame {
    ...
    void load_report(String dfrom, String dto) {
        try {
            ...
            if (all.isSelected()) {
                query = "select cname,sum(quan),sum(amount) from purchase_items where dat between ? and ?" + companyFilter + " group by cname order by cname";
                ps = conn.prepareStatement(query);
                ps.setString(1, lk);
                ps.setString(2, lk1);
            } else {
                query = "select cname,sum(quan),sum(amount) from purchase_items where dat between ? and ? and cname=?" + companyFilter + " group by cname order by cname";
                ps = conn.prepareStatement(query);
                ps.setString(1, lk);
                ps.setString(2, lk1);
    ...
    final void get_cid() {
        try {
            ...
            PreparedStatement ps = conn.prepareStatement("select count(distinct cname) from purchase_items" + companyFilter);
            r = ps.executeQuery();
            while (r.next()) {
                count = r.getInt(1);
            }
            ps = conn.prepareStatement("select distinct cname from purchase_items" + companyFilter);
            r = ps.executeQuery();
